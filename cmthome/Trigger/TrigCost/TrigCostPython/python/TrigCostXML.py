...
def WriteCsvFile(runnumber, lbrange, rates, lbset=None, xmlname=None):

    #open file / write header
    outfolder = "costMonitoring_OnlineTRPRatesLB"+lbrange+"_"+runnumber
    ...
    if xmlname:
      copyfile(xmlname, outfolder + "/xml/TrigRate_OnlineTRPRatesLB"+lbrange+"_HLT_All.xml")
    ...
    for lvl in ['L1','HLT']:
        ...
        if lvl == 'HLT':
            out = out_HLT
        out.write('Name,Time,Group,Unprescaled Rate [Hz],Weighted PS Rate Err [Hz]\n')
        out.write(',Integrated walltime,Not used here,Average rate online weighted by prescales,Statistical uncertainty on rate\n')
        ...
        for chname in rates.GetChainNames(lvl):
            ...
            if lvl == 'L1': # Unprescaled rate L1
                out.write(chname + ',' + liveTime + ',-,' + str(ch.GetAttrWithDefault("tbprate",0)) + ',' + str(ch.GetAttrWithDefault("tbprateerr",0)) + '\n' )
            elif lvl == 'HLT': # Unprescaled rate HLT
                PS = 1.0
                if chlower is not None:
                    PS = chlower.GetAttrWithDefault("prescale",1.0)
                else:
                ...
                if ( ch.GetAttrWithDefault("prescale",1.0) < 0 and ch.GetRate() > 0):
                    ...
                    PS *= ch.GetAttrWithDefault("prescale",1.0)
                out.write(chname + ',' + liveTime + ',-,' + str(PS * ch.GetRate()) + ',' + str(PS * ch.GetAttrWithDefault("rateerr",0)) + '\n' )

    out_meta.write('{' + '\n' )
    out_meta.write('  "text": "metadata",' + '\n' )
    out_meta.write('  "children": [' + '\n' )
    out_meta.write('    {"PredictionLumi": "' + str(avLumi) + '"},' + '\n' )
    out_meta.write('    {"Details": "Averaged rates from online monitoring"}' + '\n' )
    out_meta.write('  ]' + '\n' )
    out_meta.write('}' + '\n' )
